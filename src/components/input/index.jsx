...
export const MultipleImageUpload = (props) => {
  const {
    ...
    rules: {
      required: props.required ? "Image is required" : false,
      validate: (files) => {
        if (!files?.length && props.required) return "Image is required";
        return (
  ...
  const [previews, setPreviews] = useState(
    value?.length ? value.map((file) => URL.createObjectURL(file)) : []
  );
  ...
  const handleFileChange = (event) => {
    ...
    if (files.length) {
      onChange(files);
      setPreviews(files.map((file) => URL.createObjectURL(file))); // Show file previews
      props.onUpload?.(files); // Callback for additional handling
    }
  };
  ...
  return (
    <div className="flex flex-col space-y-1">
      ...
      <div className="relative border rounded-md w-full cursor-pointer bg-white p-2">
        <input
          type="file"
          accept="image/*"
          multiple
